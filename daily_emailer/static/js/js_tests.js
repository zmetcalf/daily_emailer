...
var simulated_ajax_response_three = [{"pk": 1, "model": "daily_emailer.email", "fields": {"message": "Message1", "email_group": 1, "subject": "Subject1"}}, {"pk": 2, "model": "daily_emailer.email", "fields": {"message": "Message2", "email_group": 1, "subject": "Subject2"}}, {"pk": 3, "model": "daily_emailer.email", "fields": {"message": "Message3", "email_group": 1, "subject": "Subject3"}}]
...
test("sort_and_set_emails_three_models_forwards", function() {
  $('#id_email_order').text('1,2,3');
  var ordered_list = new OrderedList;
  ordered_list.merge_order(simulated_ajax_response_three);
  deepEqual(ordered_list.get('email_list')[0].pk, 1);
  deepEqual(ordered_list.get('email_list')[1].pk, 2);
  deepEqual(ordered_list.get('email_list')[2].pk, 3);
});

test("sort_and_set_emails_three_models_backwards", function() {
  $('#id_email_order').text('3,2,1');
  var ordered_list = new OrderedList;
  ordered_list.merge_order(simulated_ajax_response_three);
  deepEqual(ordered_list.get('email_list')[0].pk, 3);
  deepEqual(ordered_list.get('email_list')[1].pk, 2);
  deepEqual(ordered_list.get('email_list')[2].pk, 1);
});

test("sort_and_set_emails_three_models_random", function() {
  $('#id_email_order').text('2,3,1');
  var ordered_list = new OrderedList;
  ordered_list.merge_order(simulated_ajax_response_three);
  deepEqual(ordered_list.get('email_list')[0].pk, 2);
  deepEqual(ordered_list.get('email_list')[1].pk, 3);
  deepEqual(ordered_list.get('email_list')[2].pk, 1);
});
